 ...
 * @return {}
 */
com.bhtec.view.util.CommonWidgets.prototype.label = function(config){
	var textField = {
		xtype:'label',
		name:config.name||'without_name',
		text:config.text||'无名称'
	};
...
com.bhtec.view.util.CommonWidgets.prototype.textField = function(config){
	   var textField = {
						...
						emptyText:config.allowBlank==undefined?(config.emptyText||''):'请输入'+(config.fieldLabel)+'...',
...
com.bhtec.view.util.CommonWidgets.prototype.cuvWindow = function(config){
		...
		var formPanel = {
			...
			style:'background-color:#FFF',
			items:[com.bhtec.view.util.CommonWidgets.prototype.fieldSet(config),config.formButton||
						{
							xtype	  : 'panel',
							layout 	  : 'column',
							border    : false,
