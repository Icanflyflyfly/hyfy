	...
	public String loginSysFirstDo(){
		...
		try {
			if (!loginService.isAdministrator(userCode)) {//非管理员
				...
				uumUser = loginService.obtainUserByUserCodePwd(
						userCode, password);
				if (uumUser == null) {
		...
		if(uumUser != null)
			getSession().put(PHONE,uumUser.getUserMobile());
